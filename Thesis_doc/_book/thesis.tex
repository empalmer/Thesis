...
Unlike in other applications where the data initially countains set of
...
be done by counting all instances of word occurance in the word vector.
...
postition on the staff defines its sound. The sound of a note depends on
...
are mostly narrower than those for Mendelssohn. We suspect the
difference in density is somewhat caused by the type of piece of Bach
and Mendelssohn. We might expect that solo pieces, like those in the
Bach data set would have lower densities then for lieder, like in the
...
Also it would be interesting to implement a windowing technique.
