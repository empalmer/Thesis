...
(Mearns et al., 2010)(mearns). Because of this, it is also more
conducive to being read into an R data frame. The below picture shows
how a basic piece of music corresponds to a .krn file.
\begin{figure}
\includegraphics[width=0.5\linewidth]{images/krnmusic} \caption{How sheet music corresponds to kern files}\label{fig:unnamed-chunk-4}
\end{figure}
Kern files are organized with columns each representing one line of
...
\NormalTok{PCA <-}\StringTok{ }\KeywordTok{prcomp}\NormalTok{(features[,}\OperatorTok{-}\DecValTok{1}\NormalTok{], }\DataTypeTok{scale =}\NormalTok{ T)}
\KeywordTok{biplot}\NormalTok{(PCA,}\DataTypeTok{scale =} \DecValTok{0}\NormalTok{)}

\NormalTok{d <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{PC =} \DecValTok{1}\OperatorTok{:}\DecValTok{9}\NormalTok{,}
                \DataTypeTok{PVE =}\NormalTok{ PCA}\OperatorTok{$}\NormalTok{sdev}\OperatorTok{^}\DecValTok{2} \OperatorTok{/}\StringTok{ }\KeywordTok{sum}\NormalTok{(PCA}\OperatorTok{$}\NormalTok{sdev}\OperatorTok{^}\DecValTok{2}\NormalTok{))}
\KeywordTok{ggplot}\NormalTok{(d, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ PC, }\DataTypeTok{y =}\NormalTok{ PVE)) }\OperatorTok{+}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{composer_m <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\KeywordTok{rep}\NormalTok{(}\StringTok{"felix"}\NormalTok{,}\KeywordTok{length}\NormalTok{(felix)),}\KeywordTok{rep}\NormalTok{(}\StringTok{"fanny"}\NormalTok{,}\KeywordTok{length}\NormalTok{(fanny)))}

\CommentTok{# Compute feature: mel ints}
\CommentTok{#mel_intsb <- map(bach,mel_ints,"piano") # each }
\CommentTok{#mel_intsf <- map(mendelssohn,mel_ints,"piece")}

\CommentTok{#compute feature: harm ints}

\CommentTok{#harm_intb <- map(bach, freq_harm_ints)}
\CommentTok{#harm_intm <- map(mendelssohn, freq_harm_ints)}

\CommentTok{# Compute feature: connsonance}
\CommentTok{#cons_b <- map(bach,consonances,"piano")}
\CommentTok{#cons_m <- map(mendelssohn,consonances,"piece")}

\CommentTok{#cons_perf <- c(map(cons_b,1),map(cons_m,1)) %>% unlist()}
\CommentTok{#cons_imp <- c(map(cons_b,2),map(cons_m,2)) %>% unlist()}
\CommentTok{#cons_dis <- c(map(cons_b,3),map(cons_m,3)) %>% unlist()}

\CommentTok{# Compute features: density}
\NormalTok{dens_f <-}\StringTok{ }\KeywordTok{map}\NormalTok{(felix,beat_density)}
\NormalTok{dens_h <-}\StringTok{ }\KeywordTok{map}\NormalTok{(fanny,beat_density) }\CommentTok{# mean then sd}

\NormalTok{dens_mean <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\KeywordTok{map}\NormalTok{(dens_f,}\DecValTok{1}\NormalTok{),}\KeywordTok{map}\NormalTok{(dens_h,}\DecValTok{1}\NormalTok{)) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{unlist}\NormalTok{()}
\NormalTok{dens_sd <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\KeywordTok{map}\NormalTok{(dens_f,}\DecValTok{2}\NormalTok{),}\KeywordTok{map}\NormalTok{(dens_h,}\DecValTok{2}\NormalTok{)) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{unlist}\NormalTok{()}

\CommentTok{# Compute features: scale degree freq}

\NormalTok{sf_f <-}\StringTok{ }\KeywordTok{map}\NormalTok{(felix,scale_degree_freq) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{unname}\NormalTok{()}
\NormalTok{sf_h <-}\StringTok{ }\KeywordTok{map}\NormalTok{(fanny,scale_degree_freq)}
\NormalTok{sf_freqs <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\KeywordTok{map}\NormalTok{(sf_f,}\DecValTok{3}\NormalTok{) ,}\KeywordTok{map}\NormalTok{(sf_h,}\DecValTok{3}\NormalTok{))}
\NormalTok{sf_}\DecValTok{1}\NormalTok{ <-}\StringTok{ }\KeywordTok{map}\NormalTok{(sf_freqs,}\DecValTok{1}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }\NormalTok{unlist}
\NormalTok{sf_}\DecValTok{2}\NormalTok{ <-}\StringTok{ }\KeywordTok{map}\NormalTok{(sf_freqs,}\DecValTok{2}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }\NormalTok{unlist}
\NormalTok{sf_}\DecValTok{3}\NormalTok{ <-}\StringTok{ }\KeywordTok{map}\NormalTok{(sf_freqs,}\DecValTok{3}\NormalTok{)}\OperatorTok{%>%}\StringTok{ }\NormalTok{unlist}
\NormalTok{sf_}\DecValTok{4}\NormalTok{ <-}\StringTok{ }\KeywordTok{map}\NormalTok{(sf_freqs,}\DecValTok{4}\NormalTok{)}\OperatorTok{%>%}\StringTok{ }\NormalTok{unlist}
\NormalTok{sf_}\DecValTok{5}\NormalTok{ <-}\StringTok{ }\KeywordTok{map}\NormalTok{(sf_freqs,}\DecValTok{5}\NormalTok{)}\OperatorTok{%>%}\StringTok{ }\NormalTok{unlist}
\NormalTok{sf_}\DecValTok{6}\NormalTok{ <-}\StringTok{ }\KeywordTok{map}\NormalTok{(sf_freqs,}\DecValTok{6}\NormalTok{)}\OperatorTok{%>%}\StringTok{ }\NormalTok{unlist}
\NormalTok{sf_}\DecValTok{7}\NormalTok{ <-}\StringTok{ }\KeywordTok{map}\NormalTok{(sf_freqs,}\DecValTok{7}\NormalTok{)}\OperatorTok{%>%}\StringTok{ }\NormalTok{unlist}

\CommentTok{#==============================================================}
\CommentTok{#==============================================================}
\CommentTok{# FEATURES create data frame}
\NormalTok{featuresm <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(composer_m,}
\NormalTok{                       dens_mean,dens_sd,}
\NormalTok{                       sf_}\DecValTok{1}\NormalTok{,sf_}\DecValTok{2}\NormalTok{,sf_}\DecValTok{3}\NormalTok{,sf_}\DecValTok{4}\NormalTok{,sf_}\DecValTok{5}\NormalTok{,sf_}\DecValTok{6}\NormalTok{,sf_}\DecValTok{7}\NormalTok{)}
\CommentTok{#==============================================================}
\end{Highlighting}
\end{Shaded}
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pairs_m <-}\StringTok{ }\KeywordTok{ggpairs}\NormalTok{(featuresm,}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{color =}\NormalTok{ composer_m))}
\NormalTok{pairs_m}
\end{Highlighting}
\end{Shaded}
\begin{Shaded}
...
\NormalTok{PCAm <-}\StringTok{ }\KeywordTok{prcomp}\NormalTok{(featuresm[,}\OperatorTok{-}\DecValTok{1}\NormalTok{], }\DataTypeTok{scale =}\NormalTok{ T)}
\KeywordTok{biplot}\NormalTok{(PCA,}\DataTypeTok{scale =} \DecValTok{0}\NormalTok{)}
...
\subsubsection{Lasso logistic}\label{lasso-logistic}
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{x <-}\StringTok{ }\KeywordTok{model.matrix}\NormalTok{(composer}\OperatorTok{~}\NormalTok{.,features)[,}\OperatorTok{-}\DecValTok{1}\NormalTok{]}
\NormalTok{y <-}\StringTok{ }\NormalTok{features[,}\DecValTok{1}\NormalTok{] }\OperatorTok{%>%}\StringTok{ }\KeywordTok{unname}\NormalTok{() }\OperatorTok{%>%}\StringTok{ }\NormalTok{as.character}

\NormalTok{grid <-}\StringTok{ }\DecValTok{10}\OperatorTok{^}\NormalTok{(}\KeywordTok{seq}\NormalTok{(}\DecValTok{10}\NormalTok{,}\OperatorTok{-}\DecValTok{2}\NormalTok{,}\DataTypeTok{length =} \DecValTok{100}\NormalTok{))}
\NormalTok{lasso.mod <-}\StringTok{ }\KeywordTok{glmnet}\NormalTok{(x,y,}\DataTypeTok{family =} \StringTok{"binomial"}\NormalTok{,}\DataTypeTok{alpha =} \DecValTok{1}\NormalTok{, }\DataTypeTok{lambda =}\NormalTok{ grid)}
...
\NormalTok{test <-}\StringTok{ }\KeywordTok{as.matrix}\NormalTok{(x[}\OperatorTok{-}\NormalTok{train,])}
\NormalTok{y.test <-}\StringTok{ }\NormalTok{y[}\OperatorTok{-}\NormalTok{train]}
\NormalTok{y.train <-}\StringTok{ }\NormalTok{y[train] }\OperatorTok{%>%}\StringTok{ }\KeywordTok{unname}\NormalTok{ () }\OperatorTok{%>%}\StringTok{ }\KeywordTok{as.character}\NormalTok{()}

\NormalTok{lasso.mod <-}\StringTok{ }\KeywordTok{glmnet}\NormalTok{(x[train,],y.train, }\DataTypeTok{family =} \StringTok{"binomial"}\NormalTok{ ,}\DataTypeTok{alpha=} \DecValTok{1}\NormalTok{,}
                    \DataTypeTok{lambda =}\NormalTok{ grid, }\DataTypeTok{thresh =} \FloatTok{1e-12}\NormalTok{)}
\NormalTok{lasso.cv.out <-}\StringTok{ }\KeywordTok{cv.glmnet}\NormalTok{(x, y,}\DataTypeTok{family =} \StringTok{"binomial"}\NormalTok{, }\DataTypeTok{alpha =} \DecValTok{1}\NormalTok{)}
\NormalTok{lasso.bestlam <-}\StringTok{ }\NormalTok{lasso.cv.out}\OperatorTok{$}\NormalTok{lambda.min}

\NormalTok{lasso.pred <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(lasso.mod, }\DataTypeTok{s =}\NormalTok{ lasso.bestlam,}
                      \DataTypeTok{newx =}\NormalTok{ test, }\DataTypeTok{type =} \StringTok{"response"}\NormalTok{)}
\NormalTok{lass <-}\StringTok{ }\KeywordTok{rep}\NormalTok{(}\StringTok{"bach"}\NormalTok{,}\KeywordTok{length}\NormalTok{(y.test))}
\NormalTok{lass[lasso.pred }\OperatorTok{>}\StringTok{ }\NormalTok{.}\DecValTok{5}\NormalTok{] <-}\StringTok{ "mendelssohn"}
\NormalTok{t <-}\StringTok{ }\KeywordTok{table}\NormalTok{(lass,y.test)}
\NormalTok{MSE_lasso <-}\StringTok{ }\NormalTok{(t[}\DecValTok{1}\NormalTok{,}\DecValTok{2}\NormalTok{]}\OperatorTok{+}\NormalTok{t[}\DecValTok{2}\NormalTok{,}\DecValTok{1}\NormalTok{])}\OperatorTok{/}\KeywordTok{sum}\NormalTok{(t)}
\NormalTok{MSE_lasso}

\CommentTok{#plot(lasso.mod, xvar = "lambda", xlim = c(-5, 0), main = "Lasso")}
\KeywordTok{plot_glmnet}\NormalTok{(lasso.mod, }\DataTypeTok{xvar =} \StringTok{"lambda"}\NormalTok{,}\DataTypeTok{xlim =} \KeywordTok{c}\NormalTok{(}\OperatorTok{-}\DecValTok{5}\NormalTok{,}\DecValTok{0}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
Unsurprisingly, since there is so few data and features currently, there
are not many results.

\subsubsection{Lasso Logistic}\label{lasso-logistic-1}
